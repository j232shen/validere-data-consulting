...
dca_portfolio_simulation <- function(prices_xts, weights_xts, inflow_amount = 1000, initial_cash = 0) {
  ...
  for (i in seq_along(dates)) {
    ...
    # cash <- 0  # all cash invested
      ...
      # FULL REBALANCING: allocate total portfolio value (holdings + new cash) according to weights
    w <- as.numeric(weights_xts[i, ])
    total_value <- sum(holdings * prices) + cash
    target_value <- total_value * w
    holdings <- target_value / prices
    cash <- 0  # all cash invested
      
    # update value of holdings due to prices changes for next period's returns (if not last period)
    if (i < length(dates)) {
...
# 3. Run your DCA simulation with these static weights
dca_static_60_40 <- dca_portfolio_simulation(clean_prices, static_weights_xts, inflow_amount = 1000, initial_cash = 0)

# 4. Calculate IRR for static 60/40
static_irr <- calculate_irr(dca_static_60_40, monthly_contribution = 1000)
print(static_irr)
```
...
```{r}
# DCA simulation
dca_full_equity <- dca_portfolio_simulation(clean_prices, results_full_equity$weights, inflow_amount = 1000, initial_cash = 0)
...
dca_values <- merge(dca_60_40, 
                    dca_full_equity, 
                    dca_mtum, 
                    dca_vttvx, 
                    all = FALSE)
colnames(dca_values) <- c("60/40", "100% Equity", "100% Momentum", "Vanguard Target 2025 Fund")

# plot dca portfolio values
...
plot.xts(
  ...
  major.ticks = "years",  # show major ticks by year
  grid.col = "lightgray"  # grid color
)
```
