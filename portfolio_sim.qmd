...
### 100% VTI (benchmark; do we beat the market?)

```{r}
# get the rebalancing dates from optimized portfolio
rebalance_dates <- index(results_60_40$weights)  # weights_xts from your optimized portfolio

# get adjusted close prices for VTI and subset to rebalance dates
vti_prices_full <- get_all_adj_close(list("VTI"), from = start_date, to = end_date)
vti_prices <- vti_prices_full[rebalance_dates, "VTI", drop = FALSE]

# weights always 1 for VTI
vti_weights <- xts(matrix(1, nrow=nrow(vti_prices), ncol=1), order.by=index(vti_prices))
colnames(vti_weights) <- "VTI"

# DCA simulation
dca_vti <- dca_portfolio_simulation(vti_prices, vti_weights, inflow_amount = cf, initial_cash = 0)

# plot results
plot(dca_vti, main = "Portfolio Value with Dollar Cost Averaging") # , ylab = "Portfolio Value ($)")

# manually calculate drawdowns from portfolio values
cummax_values <- cummax(dca_vti)
drawdowns_vti <- (dca_vti / cummax_values) - 1
plot(drawdowns_vti, main = "Drawdowns (DCA Portfolio)", ylab = "Drawdown", xlab = "Date")

# calculate IRR (money-adjusted return)
irr_vti <- calculate_irr(dca_vti, monthly_contribution = cf)
print(irr_vti)
```
...
portfolios <- list(dca_60_40, dca_full_equity, dca_mtum, dca_vttvx, dca_vti)
irrs <- list(irr_60_40$annual, irr_full_equity$annual, irr_mtum$annual, irr_vttvx$annual, irr_vti$annual)
names <- c("60/40", "100% Equity", "100% Momentum", "Vanguard Target 2025", "100% VTI")
